...
面向 **海信（Hisense）** 云端智能设备的 Home Assistant 自定义集成。空调支持控制实体；洗衣机、投影仪和冰箱目前支持基础诊断接入，后续需要根据真实云端状态和命令样本继续完善控制能力。
...
- **空调**：提供空调控制、空调专用开关、温度范围配置和诊断按钮。
- **洗衣机、投影仪、冰箱**：目前提供基础设备注册、诊断刷新按钮，以及设备类型、设备 ID、原始云端状态长度等诊断传感器。投影仪会匹配包含“投影”“激光电视”等字样的设备类型。暂不提供启动/暂停、程序选择、投影仪电源、输入源、冰箱温区设置等控制实体，因为还需要真实状态数组和 App 操作命令样本。
