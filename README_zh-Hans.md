...
面向 **海信（Hisense）** 云端智能设备的 Home Assistant 自定义集成。空调支持控制实体；洗衣机和投影仪目前支持基础诊断接入，后续需要根据真实云端状态和命令样本继续完善控制能力。
...
- **洗衣机、投影仪**：目前提供基础设备注册、诊断刷新按钮，以及设备类型、设备 ID、原始云端状态长度等诊断传感器。暂不提供启动/暂停、程序选择、投影仪电源、输入源等控制实体，因为还需要真实状态数组和 App 操作命令样本。
